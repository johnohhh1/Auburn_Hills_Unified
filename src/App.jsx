...
import { ChevronLeft, Settings, Users, BarChart3, CheckSquare, Calendar, Clock, AlertTriangle, TrendingUp, Edit3, Plus, Trash2, Eye, Shield, Utensils, Award, UserPlus, Image, ExternalLink } from 'lucide-react';
...
const externalApps = [
  {
    id: 'sidework',
    name: 'Sidework Tracker',
    description: 'Daily & weekly task verification system',
    icon: Utensils,
    color: colors.chiliNavy,
    url: 'https://605-sidework-tracker.vercel.app/',
    features: ['Bilingual Support', 'Email Integration', 'Role-based Tasks']
  },
  {
    id: 'recognition',
    name: 'Recognition Forms',
    description: 'ATL team member recognition system',
    icon: Award,
    color: colors.chiliYellow,
    url: 'https://chilihead-recognition-form.vercel.app/',
    features: ['Animated Backgrounds', 'Photo Capture', 'Auto Upload']
  },
  {
    id: 'staffing',
    name: 'Staffing Planner',
    description: 'Restaurant workforce management',
    icon: Users,
    color: colors.chiliGreen,
    url: 'https://restaurant-staffing-planner.vercel.app/',
    features: ['Schedule Optimization', 'Coverage Analysis', 'Labor Cost Tracking']
  },
  {
    id: 'onboarding',
    name: 'Onboarding System',
    description: 'New hire tracking & management',
    icon: UserPlus,
    color: colors.chiliRed,
    url: 'https://onboarding-system-two.vercel.app/',
    features: ['Progress Tracking', 'Document Management', 'Multi-location Support']
  },
  {
    id: 'gallery',
    name: 'Cloud Frame Gallery',
    description: 'Recognition display & photo management',
    icon: Image,
    color: colors.chiliRedAlt,
    url: 'https://ohhh1cloud.vercel.app/',
    features: ['Slideshow Display', 'Google Drive Sync', 'Dynamic Transitions']
  }
];
...
const getFiscalInfo = () => {
  ...
  // F26 started June 26, 2025 (Thursday) - P1 Week 1
  const fiscalYearStart = new Date('2025-06-26'); // F26 P1 W1 start date
  ...
  const currentWeekStart = getCurrentFiscalWeek(today);
  const daysBetween = Math.floor((currentWeekStart - fiscalYearStart) / (1000 * 60 * 60 * 24));
  const weeksSinceFiscalStart = Math.floor(daysBetween / 7) + 1; // +1 because week 1 starts on day 0
  
  // Period structure: P1,P4,P7,P10 = 5 weeks; all others = 4 weeks
  const getPeriodWeeks = (periodNumber) => {
    return (periodNumber === 1 || periodNumber === 4 || periodNumber === 7 || periodNumber === 10) ? 5 : 4;
  };
  
  // Calculate current period and week within period
  let currentPeriod = 1;
  let weekCounter = weeksSinceFiscalStart;
  
  for (let period = 1; period <= 12; period++) {
    const weeksInThisPeriod = getPeriodWeeks(period);
    
    if (weekCounter <= weeksInThisPeriod) {
      currentPeriod = period;
      break;
    }
    weekCounter -= weeksInThisPeriod;
  }
  
  const currentWeekInPeriod = weekCounter;
  const totalWeeksInPeriod = getPeriodWeeks(currentPeriod);
  
  // Calculate week end date (Wednesday)
  ...
  return {
    ...
    overallWeek: weeksSinceFiscalStart,
    weekStart: currentWeekStart.toLocaleDateString('en-US', { month: 'numeric', day: 'numeric', year: 'numeric' }),
...
const getRelativeDate = (daysFromToday) => {
  const date = new Date(); // Use actual current date instead of hardcoded
      ...
      <div className="min-h-screen" style={{ backgroundColor: colors.chiliCream }}>
        ...
        <div className="text-white p-6" style={{ background: `linear-gradient(135deg, ${colors.chiliRed}, ${colors.chiliRedAlt}, ${colors.chiliYellow})` }}>
          <div className="flex justify-between items-start mb-4">
            <div>
              <h1 className="text-2xl font-bold mb-1">🌶️ ChiliHead Operations Hub</h1>
        ...
        {/* Core ChiliHead Functions */}
        <div className="p-6 space-y-4">
          <h2 className="text-xl font-bold mb-4" style={{ color: colors.chiliNavy }}>🌶️ Core Operations</h2>
        ...
        {/* External Applications Hub */}
        <div className="p-6">
          <div className="bg-white rounded-lg p-6 shadow-md">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-bold" style={{ color: colors.chiliNavy }}>🔗 Restaurant Applications</h2>
            ...
            <p className="text-sm mb-6" style={{ color: colors.chiliBrown }}>
              Launch external applications to manage specific restaurant operations
            </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {externalApps.map(app => (
                <button
                  ...
                  className="bg-gray-50 hover:bg-gray-100 rounded-lg p-4 text-left transition-all duration-200 border border-gray-200 hover:border-gray-300 hover:shadow-md group"
                >
                  ...
                  <h3 className="font-bold text-lg mb-2" style={{ color: colors.chiliNavy }}>
                    {app.name}
                  </h3>
                  <p className="text-sm mb-3" style={{ color: colors.chiliBrown }}>
                    {app.description}
                  </p>
